...
if( isset( $_POST['btnSave'] ) ){
    ...
    */
    ...
    $row = $db->get_row('appointments',array('staff_id'=>$staff_id,'date' => $date ,'time_slot' => $time_slot));
    if ( $row  !=  null ){
      $form_errors['unavailable'] = "Slot unavailable";
    }
    ...
    if( count($form_errors) >0 ){
      ...
      $amount = $db->get_colum_value('services','amount',array('id' => $service_id ));
      $sql = "INSERT INTO `appointments`( `customer_id`, `service_id`,`staff_id`, `date`, `time_slot`, `notes`, `booked_on`,`amount`,`payment_status`, `status`) VALUES ('".$customer_id."','".$service_id."','".$staff_id."','".$date."','".$time_slot."','".$notes."','".$booked_on."','".$amount."','0','1')";
      if ( $db->execute($sql) ){
          $session->set_flashdata('msg',alert('Appoinment booked successfully','success'));
          redirect('payment.php?appointment_id='.$db->get_insert_id());
    ...
    <section id="contact" class="contact" style="margin-top: 50px;min-height: 600px">
      <div class="container" data-aos="fade-up">
        ...
        <div class="row" data-aos="fade-up" data-aos-delay="100">
          <div class="col-lg-8 offset-lg-2">
            <div class="box-1">
             <spane style="color:red"><?= form_error('unavailable'); ?></spane> 
              ...
              <form action="<?= base_url("appointments.php") ?>" method ="post">
                ...
                <div class="form-group row">
                  ...
                  <select class="form-control" name="staff_id" id="staff_id">
                      ...
                      <option value="<?= $row['id'] ?>" <?= $staff_id == $row['id'] ? ' selected="selected"' :''  ?> ><?= $row['name'] ?></option>
                      <?php } ?>
                    </select>
                    <?= form_error('staff_id'); ?>
                ...
                <div class="form-group row">
                  ...
                  <div class="col-sm-5">
                    <button type="submit" name="btnSave" class="btn btn-success">Book Now</button>
                  </div>
                </div>
              </form>
            </div>
