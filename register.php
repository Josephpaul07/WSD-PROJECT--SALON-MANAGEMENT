...
if ( isset( $_POST['btnRegister'] ) ){
  ...
  */
  ...
  if( count($form_errors) >0 ){
    ...
    $proof = "";
    if( $_FILES['fl_proof']['name'] != "" ) {
      $upload->config['max_size'] = 3000000;
      $upload->config['allowed_types'] = 'jpg|png|jpeg|gif|jfif';
      $upload->config['upload_path'] = 'uploads/proofs/';
      if ( $upload->do_upload('fl_proof') ){
        if($proof  != '' && file_exists(base_path( $proof ) )){
            unlink(base_path( $proof ));
        }
        $proof = $upload->upload_info['file_name'];
      }else{
          $form_errors['proof'] = $upload->upload_info['error']; 
          $process = 'FORM';
          goto htmlView;
      }
  }
    ...
    $sql = "INSERT INTO `customers`(`user_id`, `name`, `address`, `phone`, `email`, `gender`, `proof`, `reg_date`) VALUES ('".$user_id."','".$name."','".$address."','".$phone."','".$email."','".$gender."','".$proof."','".$reg_date."')";
    ...
    <section id="contact" class="contact" style="margin-top: 50px;min-height: 600px">
      <div class="container" data-aos="fade-up">
        ...
        <div class="row" data-aos="fade-up" data-aos-delay="100">
          <div class="col-lg-8 offset-lg-2">
            <div class="box-1">
              <form action="<?= base_url("register.php") ?>" method ="post" enctype="multipart/form-data">
